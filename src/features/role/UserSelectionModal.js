...
function UserSelectionModal({ showModal, closeModal, roleId, onUserAssigned }) {
  ...
  const handleAssignRole = async () => {
    ...
    try {
      // Gọi service để gán role cho người dùng
      await UserService.assignRoleToUsers(roleId, selectedUserIds);
      dispatch(showNotification({ message: 'Người dùng đã được gán role thành công!', status: 1 }));
      onUserAssigned();
      closeModal();
    } catch (error) {
      console.error("Lỗi khi gán role:", error);
      dispatch(showNotification({ message: 'Đã xảy ra lỗi khi gán role.', type: 'error' }));
    }
  };
      ...
      {showModal && (
        <div className="modal modal-open">
          <div className="modal-box">
            <h3 className="font-bold text-lg mb-4">Chọn người dùng để gán role</h3>
