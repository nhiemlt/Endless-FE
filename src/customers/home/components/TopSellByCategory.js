...
function TopSellByCategory() {
  ...
  const fetchTopSellingProductsByCategory = async categoryID => {
    try {
      const response =
        await productVersionService.getTopSellingProductsByCategory(categoryID)
  ...
  const fetchAllProductsByCategories = async () => {
    ...
    for (const category of categories) {
      const products = await fetchTopSellingProductsByCategory(
        category.categoryID
      )
  ...
  return (
    <div>
      {loadingCategories ? (
        ...
        categories
          .filter(
            category => productsByCategory[category.categoryID]?.length > 0
          ) // Lọc các danh mục có sản phẩm
          .map(category => (
            <section
              key={category.categoryID}
              className='p-3 dark:bg-base-100 bg-white'
            >
              <div className='container'>
                ...
                <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 mt-3 dark:bg-base-200'>
                  {productsByCategory[category.categoryID].map(product => (
                    ...
                    >
                      <div className='w-fulll h-32 bg-white'>
                        <img 
                          src={product.image}
                          alt={product.product.name}
                          className='inset-0 w-auto h-32 object-cover transition duration-500 group-hover:scale-105 mx-auto'
                      ...
                      <div className='relative border border-gray-100 dark:border-gray-800 bg-blue-50 dark:bg-base-200 p-4'>
                        <p
                          className='mt-1 text-sm text-gray-900 h-10 overflow-hidden transform scale-95 dark:text-white'
