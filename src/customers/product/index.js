    ...
    <div className='flex flex-col lg:flex-row'>
      ...
      <div className='w-full lg:w-1/4 p-4 dark:bg-base-100 bg-white rounded-lg shadow-md'>
        ...
        <div>
          <label
            htmlFor='SortBy'
            className='block text-xs font-medium dark:text-white text-gray-900 mt-3'
          >
        ...
        <div>
          <label
            htmlFor='SortBy'
            className='block text-xs font-medium dark:text-white text-gray-900 mt-5'
          >
            <b>Sắp xếp</b>
          </label>
          <select
          ...
          >
            ...
            <option value='discountPrice, ASC'>
              Giá khuyến mãi - Tăng dần
            </option>
            <option value='discountPrice, DESC'>
              Giá khuyến mãi - Giảm dần
            </option>
            <option value='numberOfReviews, ASC'>
              Lượt đánh giá - Tăng dần
            </option>
            <option value='numberOfReviews, DESC'>
              Lượt đánh giá - Giảm dần
            </option>
                    ...
                    categories.map(category => (
                      <li key={category.categoryID}>
                        <label className='inline-flex items-center gap-2'>
                          <input
                            type='checkbox'
                            checked={filterData.categoryIDs.includes(
                              category.categoryID
                      ...
                      <li key={brand.brandID}>
                        <label className='inline-flex items-center gap-2'>
                          <input
                            type='checkbox'
                            value={brand.brandID}
                            checked={filterData.brandIDs.includes(
                              brand.brandID
                            )} // Kiểm tra xem thương hiệu có được chọn không
                            onChange={() => {
            ...
            {products.map((product, index) =>
              index < 8 ? (
                <div
                  ...
                  onClick={() => handleViewDetails(product.productID)} // Chuyển trang khi bấm vào sản phẩm
                >
                  {/* Hình ảnh sản phẩm */}
                  <div className='relative w-full h-32'>
                    <img
                      src={
                        product.productVersionDTOs[0]?.image ||
                        `https://www.lg.com/lg5-common/images/common/product-default-list-350.jpg`
                      }
                      className='absolute inset-0 w-auto h-32 object-cover transition duration-500 group-hover:scale-105 mx-auto'
                      alt={product.name}
                    />
                    {/* Giảm giá */}
                    {product.discountPercentage > 0 && (
                      <p className='absolute left-2 bg-red-600 text-white font-semibold text-xs px-2 py-1 rounded-md shadow-md'>
                  ...
                  <div className='relative border border-gray-100 dark:border-gray-800 bg-white dark:bg-base-200 p-4'>
                    ...
                    formatCurrency(product.discountPrice) ? (
                      <span className='mt-1 text-sm text-red-600 max-h-10'>
      ...
      {/* Phân trang */}
      <div className="join flex justify-center mt-3 bg-base-100 p-2">
        <button
        ...
        {page > 2 && (
          <>
            <button className="join-item btn" onClick={() => changePage(0)}>
              1
            </button>
            {page > 3 && (
              <button className="join-item btn" onClick={() => changePage(page - 3)}>
                ...
              </button>
            )}
          </>
        ...
        {Array.from({ length: 5 }, (_, i) => page - 2 + i)
          ...
          .map(p => (
            <button
              key={p}
              className={`join-item btn ${p === page ? 'btn-active bg-info' : ''}`}
        ...
        {page < totalPages - 3 && (
          <>
            ...
            <button className="join-item btn" onClick={() => changePage(totalPages - 1)}>
              {totalPages}
            </button>
          </>
        )}
